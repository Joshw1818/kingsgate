...
| 2.5 | Agency revenue & retention — MRR, avg retainer (£360.58 actual), churn, channel split. Reads the seeded `clients` ledger only | NOT STARTED |
| 2.6 | Client Tracker tab — lifecycle stage + billing arrangement + due dates, all computed; manual "mark paid" → `billing_schedule` (Stripe-ready) | NOT STARTED |
